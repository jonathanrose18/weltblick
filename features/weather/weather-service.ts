...
export async function getWeatherForCountry(countryName: string): Promise<WeatherData> {
  const sanitizedCountryName = countryName.trim().toLowerCase();
  const countriesResponse = await countriesClient.get<Country[]>(`/name/${sanitizedCountryName}?fullText=true`);

  const countryData = countriesResponse.data[0];
  const latlng = countryData?.capitalInfo?.latlng;

  if (!latlng || latlng.length !== 2) {
    throw new NotFoundError(`No location data found for country: ${sanitizedCountryName}`);
  }
